  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('Students', style: TextStyle(fontSize: 24, fontWeight: FontWeight.w700, color: kGray900)),
        ...
        LayoutBuilder(builder: (ctx, c) => GridView.count(
          crossAxisCount: c.maxWidth > 600 ? 4 : 2, shrinkWrap: true, physics: const NeverScrollableScrollPhysics(),
          ...
          children: [
            ...
            _stat('CS Program', '${students.where((s) => s.program == 'BS Computer Science').length}', kBlue),
            _stat('IT Program', '${students.where((s) => s.program == 'BS Information Technology').length}', const Color(0xFF7C3AED)),
          ],
        )),
        const SizedBox(height: 20),
        Container(
          ...
          child: SingleChildScrollView(scrollDirection: Axis.horizontal, child: DataTable(
            ...
            rows: students.map((s) => DataRow(cells: [
              DataCell(Text(s.studentId, style: const TextStyle(fontSize: 13, color: kGray900))),
              DataCell(Text(s.name, style: const TextStyle(fontSize: 13, color: kGray900))),
              DataCell(Text('📧 ${s.email}', style: const TextStyle(fontSize: 12, color: kGray500))),
              DataCell(Text(s.program, style: const TextStyle(fontSize: 13, color: kGray500))),
              DataCell(Text(s.yearLevel, style: const TextStyle(fontSize: 13, color: kGray500))),
              DataCell(Container(
                padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                decoration: BoxDecoration(color: s.status == 'Active' ? kGreenLight : kGray100, borderRadius: BorderRadius.circular(20)),
                child: Text(s.status, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: s.status == 'Active' ? kGreenDark : kGray700)),
              )),
            ])).toList(),
          )),
        ),
